...
layerRules.Add("ACTIVATION_FUNCTION", (fun name input -> if input.GetType() <> typeof<string> then
...
layerRules.Add("OUTPUT_FUNCTION", (fun name input ->  if input.GetType() <> typeof<string> then
                                                        invalidArg name "Wrong type, expected 'string'"))
...
type MultiLayerNetworkBuilder() =
    ...
    let createHiddenLayersList (builder:Builder<'T>) =
        if builder.GetAspects("HIDDEN_LAYER") = Seq.empty then
            ...
            |> Seq.map (fun pStore -> let neurons : int = pStore.GetValues("NEURONS") |> Seq.exactlyOne |> unbox  
                                      let actFunStr = pStore.GetValues("ACTIVATION_FUNCTION") |> Seq.exactlyOne |> unbox                                  
                                      let actFun : ActivationFunType = buildActivationFunction actFunStr
                                      let outFunStr = pStore.GetValues("OUTPUT_FUNCTION") |> Seq.exactlyOne |> unbox            
                                      let outFun : OutputFunType = buildOutputFunction outFunStr
    ...
    let createOutputLayer (builder:Builder<'T>) =
        if builder.GetAspects("OUTPUT_LAYER") = Seq.empty then
            ...
            let actFunStr = outStore.GetValues("ACTIVATION_FUNCTION") |> Seq.exactlyOne |> unbox                                  
            let actFun : ActivationFunType = buildActivationFunction actFunStr
            let outFunStr = outStore.GetValues("OUTPUT_FUNCTION") |> Seq.exactlyOne |> unbox            
            let outFun : OutputFunType = buildOutputFunction outFunStr
            Some(actFun, outFun)
